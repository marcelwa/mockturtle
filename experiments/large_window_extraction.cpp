...
int main()
{
  constexpr uint16_t MIN_CUT_SIZE = 8;
  constexpr uint16_t MAX_CUT_SIZE = 14;
  constexpr uint16_t MIN_NUM_LEVELS = 12;
  constexpr uint16_t MAX_NUM_LEVELS = 30;
  constexpr uint16_t MIN_WINDOW_SIZE = 200;

  // Create directory structure if it doesn't exist
  fs::path windows_dir = "benchmarks/large_windows";
  ...
  for ( auto const& benchmark : experiments::all_benchmarks( experiments::epfl ) )
  {
      ...
      for ( auto num_levels = MIN_NUM_LEVELS; num_levels <= MAX_NUM_LEVELS; ++num_levels )
      {
        ...
        topo_aig.foreach_gate( [&topo_aig, &windowing, &window_index, &benchmark_name,
          ...
          if ( const auto w = windowing.run( n, cut_size, num_levels ); w.has_value() )
          {
            ...
            if (window.num_gates() < MIN_WINDOW_SIZE)
            {
              return;
            }
