...
/* ------------------------------------------------------------------------- *
 * ACD instrumentation (agentic-synthesis).                                   *
 *                                                                            *
 * Entirely inert unless the environment variable ACD_STATS names a file.      *
 * When enabled, every call to ac_decomposition_impl::run() appends one CSV    *
 * record describing the column-multiplicity landscape that was searched, so   *
 * that the headroom left by the feasibility test mu <= 2^(k-|FS|) can be      *
 * measured rather than guessed at.                                            *
 * ------------------------------------------------------------------------- */
struct acd_stats_log
{
  static FILE*& handle()
  {
  ...
  static bool enabled()
  {
    ...
    if ( state < 0 )
    {
      const char* path = std::getenv( "ACD_STATS" );
      if ( path != nullptr && path[0] != '\0' )
      {
        handle() = std::fopen( path, "a" );
        state = handle() != nullptr ? 1 : 0;
        if ( state == 1 )
        {
          std::fprintf( handle(), "#nvars,late,use_first,ok,best_fs,best_mu,num_luts,fs,min_mu,thr,exact\n" );
        }
      }
      else
      {
...
/*! \brief Free-set width budget, overridable from the environment.
 *
 * Upstream ABC fixes the maximum free-set size at 4 (relaxed only when the support is so
 * wide that the bound set would not fit a k-LUT).  For k = 6 that permanently excludes
 * |FS| = 5, which is the only configuration that yields a two-LUT decomposition of a
 * 9- to 11-variable cut, and it also causes every cut with five or more late-arriving
 * variables to be rejected without any search.  ABC's comment says the exclusion pays for
 * itself "on average"; that is a runtime argument for a general-purpose mapper, and it is
 * not an argument that applies to a record attempt.  ACD_MAX_FS lifts the cap.
 */
inline uint32_t acd_max_free_set_vars( uint32_t dflt = 4u )
{
  static int cached = -1;
  if ( cached < 0 )
  {
    const char* s = std::getenv( "ACD_MAX_FS" );
    cached = ( s != nullptr && s[0] != '\0' ) ? std::atoi( s ) : 0;
    if ( cached < 1 || cached > 5 )
      cached = 0;
  }
  return cached > 0 ? static_cast<uint32_t>( cached ) : dflt;
}
...
struct ac_decomposition_params
{
  ...
  uint32_t max_free_set_vars{ acd_max_free_set_vars( 4 ) };
...
public:
  ...
  int run( word* ptt, unsigned delay_profile )
  {
    ...
    {
      acd_log_reject( 1, __builtin_popcount( delay_profile ) );
      return -1;
    }
    ...
    {
      acd_log_reject( 2, late_arriving );
    ...
    {
      acd_log_reject( 3, late_arriving );
      return -1;
    }
    ...
    {
      stat_min_mu[i] = UINT32_MAX;
      stat_mu_exact[i] = true;
    }

    /* run ACD trying different bound sets and free sets */
    bool const ok = find_decomposition( delay_profile, late_arriving );

    acd_log_run( ok, late_arriving );
  ...
  /*! \brief Instrumentation hook: a call rejected before any search was done.
   *
   * reason 1 = support wider than the 11-variable static truth table,
   * reason 2 = more late-arriving variables than the free set may hold,
   * reason 3 = more late-arriving variables than lut_size - 1.
   */
  void acd_log_reject( int reason, uint32_t late_arriving )
  {
    if ( !acd_stats_log::enabled() )
      return;

    std::fprintf( acd_stats_log::handle(), "%u,%u,%d,-%d,-1,-1,0,0,0,0,1\n",
                  num_vars, late_arriving, ps.use_first ? 1 : 0, reason );
  }

  /*! \brief Instrumentation hook: one CSV record per attempted decomposition. */
  void acd_log_run( bool ok, uint32_t late_arriving )
  {
    if ( !acd_stats_log::enabled() )
      return;

    FILE* f = acd_stats_log::handle();
    uint32_t const num_luts = ok ? ( best_multiplicity <= 2 ? 2 : best_multiplicity <= 4 ? 3
                                                               : best_multiplicity <= 8 ? 4
                                                                                        : 5 )
                                 : 0;

    bool any = false;
    for ( uint32_t fs = 1; fs < 8; ++fs )
    {
      if ( stat_min_mu[fs] == UINT32_MAX )
        continue;
      any = true;
      uint32_t const thr = fs < ps.lut_size ? ( 1u << ( ps.lut_size - fs ) ) : 0u;
      std::fprintf( f, "%u,%u,%d,%d,%d,%d,%u,%u,%u,%u,%d\n",
                    num_vars, late_arriving, ps.use_first ? 1 : 0, ok ? 1 : 0,
                    ok ? (int)best_free_set : -1, ok ? (int)best_multiplicity : -1,
                    num_luts, fs, stat_min_mu[fs], thr, stat_mu_exact[fs] ? 1 : 0 );
    }

    if ( !any )
    {
      std::fprintf( f, "%u,%u,%d,%d,%d,%d,%u,0,0,0,1\n",
                    num_vars, late_arriving, ps.use_first ? 1 : 0, ok ? 1 : 0,
                    ok ? (int)best_free_set : -1, ok ? (int)best_multiplicity : -1, num_luts );
    }
  }
...
private:
  ...
  std::tuple<STT, std::array<uint32_t, max_num_vars>, uint32_t> enumerate_iset_combinations( uint32_t free_set_size, uint32_t offset, Fn&& fn )
  {
    ...
    {
      best_cost = fn( tt );
      stat_min_mu[free_set_size] = std::min( stat_min_mu[free_set_size], best_cost );
    ...
    {
      uint32_t cost = fn( tt );
      if ( cost < stat_min_mu[free_set_size] )
      {
        stat_min_mu[free_set_size] = cost;
      }
      if ( cost < best_cost )
      {
        local_best_tt = tt;
        ...
        {
          stat_mu_exact[free_set_size] = false;
          break;
        }
      }
  ...
  inline std::tuple<STT, std::array<uint32_t, max_num_vars>, uint32_t> enumerate_iset_combinations2( uint32_t free_set_size, uint32_t offset )
  {
    ...
    /* column_multiplicity2 saturates at 3, so the recorded minimum is a lower bound only */
    stat_mu_exact[free_set_size] = false;
...
private:
  ...
  /* instrumentation: true (unclamped) minimum column multiplicity seen per free-set size */
  uint32_t stat_min_mu[8] = { UINT32_MAX, UINT32_MAX, UINT32_MAX, UINT32_MAX,
                              UINT32_MAX, UINT32_MAX, UINT32_MAX, UINT32_MAX };
  /* whether that minimum is exact (full enumeration) or truncated by an early bail-out */
  bool stat_mu_exact[8] = { true, true, true, true, true, true, true, true };
};
